...
$(function() {
    ...
    $('.lockerroom-response-control').click(function() {
        var $id = $(this).data('id');
        $('.lockerroom-response-control').removeClass('active');
        $(this).addClass("active");
        $('.response').removeClass('active');
        $('body').addClass('lockerroom-responses-up');
        $('.response#' + $id).addClass('active');
    });

    $('.close-lockerroom-responses').click(function() {
        $('.lockerroom-response-control').removeClass('active');
        $('.response').removeClass('active');
        $('body').removeClass('lockerroom-responses-up');
    });


    $('.next-lockerroom-response').click(function() {
        moveNext();
    });

    function moveNext() {
        var total = $('.lockerroom-response-control').length;
        var current = $('.lockerroom-response-control').index($('.lockerroom-response-control.active'));

        $('.lockerroom-response-control').removeClass('active');
        $('.row.response').removeClass('active');

        var next = current + 1;
        if (next < total) {} else {
            next = 0;
        }
        $('.row.response').eq(next).addClass('active');
        $('.lockerroom-response-control').eq(next).addClass('active');
    }

    $('.prev-lockerroom-response').click(function() {
        movePrev();
    });
